...
###output files###
#write.csv(max_lev[,c("studyid","gender","DX","age","final_ratio")], paste("tdjv_completers_",as.character(Sys.Date()),".csv",sep=""), row.names=F)
#write.csv(nc_data, paste("tdjv_noncompleters_",as.character(Sys.Date()),".csv",sep=""),row.names=F)

###Statistics/Machine Learning###

fit1<-lm(final_ratio~DX2*gender*age_c, data=max_lev)
fit2<-lm(final_ratio~DX2, data=max_lev)

###Collapsed final_ratio
...
legend(0.95, 20, c("22q n=36", "td n=30", "xxx n=11", "xxy n=12", "sca n=23"))

setwd("~/Documents/Lab/AMC APC TDJ/NewTables")
library(stargazer)
library(car)
stargazer(summary(fit4)[4], summary=F, digits=2, out="fit2v_collapsed.html")
#stargazer(Anova(fit2), summary=F, digits=2, out="tdjv_Anova.html")
#stargazer(pairwise.t.test(max_lev$final_ratio, max_lev$DX2, p.adj="none")[3], summary=F, digits=2, out="tdjv_none.html")
#stargazer(pairwise.t.test(max_lev$final_ratio, max_lev$DX2, p.adj="fdr")[3], summary=F, digits=2, out="tdjv_fdr.html")
